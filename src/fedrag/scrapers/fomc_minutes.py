...
class FOMCMinutesScraper(BaseScraper):
    """Scraper for FOMC meeting minutes."""

    CALENDAR_URL = "/monetarypolicy/fomccalendars.htm"
    MINUTES_PATTERN = re.compile(r"/monetarypolicy/fomcminutes\d+\.htm")
    ...
    async def get_document_urls(
        ...
        """Get FOMC minutes URLs from the calendar page.

        Minutes links match pattern: /monetarypolicy/fomcminutesYYYYMMDD.htm
        """
        ...
        for link in soup.find_all("a", href=True):
            href = link["href"]
            if self.MINUTES_PATTERN.match(href):
                full_url = self.make_absolute_url(href)

                # Extract year from URL to filter
                year_match = re.search(r"/fomcminutes(\d{4})", href)
                if year_match:
    ...
    def _extract_date_from_url(self, url: str) -> Optional[date]:
        """Extract date from minutes URL.

        URL format: /monetarypolicy/fomcminutes20231101.htm
        """
    ...
    def _extract_content(self, soup) -> str:
        """Extract minutes content from parsed HTML."""
        # Minutes are typically in the main content area
    ...
    def _extract_title(self, soup) -> str:
        """Extract minutes title from parsed HTML."""
        h1 = soup.find("h1")
        if h1:
    ...
    async def scrape_document(self, url: str) -> Optional[FedDocument]:
        """Scrape a single FOMC minutes document."""
        logger.info(f"Scraping minutes: {url}")
        ...
        try:
            ...
            return FedDocument.create(
                ...
                content=content,
                raw_html=html,
                speaker=None,  # Committee document
                has_pdf=has_pdf,
                pdf_url=pdf_url,
            )

        except Exception as e:
            logger.error(f"Error scraping minutes {url}: {e}")
            raise
